...
- Commands with safe preview or quote modes default to the preview/quote output; pass `--yes` to submit the transaction. `--preview` and `--quote-only` remain explicit aliases for those read-only modes.
...
rare collection deploy lazy-erc721 <name> <symbol> --max-tokens <number> [--contract-type <type>] [--chain <chain>] [--chain-id <id>] [--yes]
...
rare liquid-edition set-render-contract --contract <address> --render-contract <address> [--yes] [--chain <chain>] [--chain-id <id>]
...
rare listing release configure --contract <address> --price <amount> --max-mints <number> [--currency <currency>] [--start-time <time>] [--split <addr=ratio>] [--yes] [--chain <chain>] [--chain-id <id>]
...
rare listing release allowlist set --contract <address> --end-time <time> [--input <file>] [--root <bytes32>] [--yes] [--chain <chain>] [--chain-id <id>]
rare listing release allowlist clear --contract <address> [--yes] [--chain <chain>] [--chain-id <id>]
rare listing release limits set-mint --contract <address> --limit <number> [--yes] [--chain <chain>] [--chain-id <id>]
rare listing release limits set-tx --contract <address> --limit <number> [--yes] [--chain <chain>] [--chain-id <id>]
...
rare listing batch cancel [--input <path>] [--root <hex>] [--contract <address>] [--token-id <id>] [--yes] [--chain <chain>] [--chain-id <id>]
rare listing batch set-allowlist [--input <path>] [--root <hex>] [--contract <address>] [--token-id <id>] [--allowlist-root <hex>] [--end-time <unix>] [--yes] [--chain <chain>] [--chain-id <id>]
