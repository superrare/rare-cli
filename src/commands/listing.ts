  ...
  cmd
    ...
    .action(async (opts: ListingCreateOptions): Promise<void> => {
      ...
      try {
        ...
        const target = opts.target === undefined ? PUBLIC_LISTING_TARGET : parseAddress(opts.target, '--target');
    ...
    .action(async (opts: ListingCancelOptions): Promise<void> => {
      ...
      if (opts.collection !== undefined) {
        ...
        const result = await rare.collectionMarket.listing.getStatus({
          ...
          seller: parseAddress(opts.seller, '--seller'),
          tokenId: opts.tokenId,
          account,
        });
