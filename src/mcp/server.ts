...
import { createPublicClient, createWalletClient, http, isAddress, type PublicClient } from 'viem';
...
function registerToolByName(server: McpServer, name: McpToolName): void {
  ...
  switch (name) {
    case 'config_summary':
      ...
      }, () => toolResult(shapeMcpConfigSummary(readConfig())));
      return;
    case 'wallet_address':
      server.registerTool(name, {
    ...
    case 'token_contract_status':
      server.registerTool(name, {
        ...
        annotations: { readOnlyHint: true, openWorldHint: true },
      }, async ({ chain, contract }) => toolResult(await rareForChain(chain).token.getContractInfo({ contract })));
      return;
    case 'token_status':
      server.registerTool(name, {
        ...
        annotations: { readOnlyHint: true, openWorldHint: true },
      }, async ({ chain, contract, tokenId }) => toolResult(await rareForChain(chain).token.getTokenInfo({ contract, tokenId })));
      return;
    case 'token_price':
      server.registerTool(name, {
    ...
    case 'auction_status':
      server.registerTool(name, {
        ...
        inputSchema: { ...optionalChain, contract: addressSchema, tokenId: tokenIdSchema },
        annotations: { readOnlyHint: true, openWorldHint: true },
      }, async ({ chain, contract, tokenId }) => toolResult(await rareForChain(chain).auction.getStatus({ contract, tokenId })));
      return;
    case 'offer_status':
      server.registerTool(name, {
  ...
  switch (name) {
    ...
    case 'import_erc721':
      server.registerTool(name, {
        ...
        await writeRare(chain).import.erc721({ contract, owner });
...
function mediaEntrySchema(): ReturnType<typeof z.object> {
  return z.object({
    url: z.string(),
    mimeType: z.string(),
...
function getPublicClient(chain: SupportedChain): PublicClient {
  const config = readConfig();
...
function shapeTransactionResult(value: unknown, extra: Record<string, unknown> = {}): unknown {
  ...
  const block = isRecord(receipt) && 'blockNumber' in receipt
    ? { blockNumber: receipt.blockNumber }
    : {};

  return { ...rest, ...block, ...extra };
}
...
function isMcpWriteToolName(name: McpToolName): name is McpWriteToolName {
  return mcpWriteToolNames.some((toolName) => toolName === name);
}
